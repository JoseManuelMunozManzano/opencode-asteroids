...
- `game.js` — whole game, `'use strict'`, globals (`ship, bullets, asteroids, particles, powerUps, shootingStars, score, lives, level, state, deadTimer, shootingStarTimer, currentSkinId`). Fixed `W=800, H=600`.
- `favicon.svg` — static only.

## Game logic (`game.js`)

- Loop: `initGame()` + `requestAnimationFrame(loop)`, dt clamped to 0.05s.
- Input via `keys` (held) + `justPressed`/`pressed(code)` (one-shot, consumed on read). Uses `e.code`: `ArrowLeft/Right/Up`, `Space`, `KeyC` (cambio de skin, en cualquier estado).
...
- Power-ups (`powerUps`): 15% drop compartido en asteroide destruido, 50/50 `SpeedPowerUp` vs `TripleShotPowerUp`, más 10% independiente `ShieldPowerUp`, pickup ttl 8s, radius 12, pickup usa `ship.radius + p.radius`. Velocidad/triple duran 5s, escudo 10s; re-pickup resetea timer, cleared on `Ship.reset()` (death/respawn/level change); `powerUps` cleared in `initGame()`/`nextLevel()`.
...
- Power-up Escudo (`ShieldPowerUp`): 10% independent drop on destroyed asteroid, pickup ttl 8s, effect 10s. `Ship.activateShield()` sets numeric `shield` timer (re-pickup resets, no stacking); `Ship.update()` decrements, expiry clears to 0. Cleared on `reset()`. `Ship.draw()` renders ring at `radius + 8` while `shield > 0`; HUD shows `ESCUDO` + remaining seconds, stacked below velocidad/triple. Destruction centralized in `destroyAsteroid()` / `destroyShootingStar()` via `dropPowerUps()` so bullet kills and shield absorbs share points, split, and drops (15% compartido velocidad/triple + 10% escudo). Ship collision with shield destroys hazard with normal reward without consuming timer, filters `asteroids`/`shootingStars`; without shield calls `killShip()`. Invincibility retains priority over shield.
...
- Skins de nave (`SHIP_SKINS`, `currentSkinId`, `getSkin()/setSkin()/cycleSkin()`): 3 siluetas monocromas (`clasica`, `interceptor`, `platillo`), solo visual. `KeyC` rota al inicio de `update()`, funciona en `playing | dead | gameover`. Selección global fuera de `Ship`: `reset()`/`nextLevel()`/`initGame()` la conservan. Persistencia en `localStorage` (`asteroids-skin`) con validación y `try/catch`. `Ship.draw()` delega silueta + llama (`skin.path()`, `skin.flameX`); `drawLifeIcon()` reutiliza `skin.path()` a escala 0.45. Radio, física, disparo, hitbox intactos. HUD inferior: `SKIN <NOMBRE>  [C]`.
